...
//	ajaxRequest.open('POST', '/XProtectMobile/Communication');
	ajaxRequest.open('POST', 'http://localhost:8081/XProtectMobile/Communication');
...
var isResponseOk = function(response) {
	...
	if (result == 'OK') {
		...
		console.log('Command ' + commandName + ' finished with an error.');
		...
		if (response.querySelector('Command ErrorString')) {
			alert(response.querySelector('Command ErrorString').firstChild.textContent);
		} else if (response.querySelector('Command ErrorCode')) {
			alert('ErrorCode ' + response.querySelector('Command ErrorCode').firstChild.textContent + ' for command ' + commandName);
		}
